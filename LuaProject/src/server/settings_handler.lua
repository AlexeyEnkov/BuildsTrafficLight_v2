return function(c, data, cb)
    ...
    if rConf then
        ...
        if k then
            ...
            if nConf.ssid ~= _G["cfg"].ssid then
                wifiChanged = true
            end
            if (nConf.pass and #nConf.pass >= 8) then
                wifiChanged = true oldC.pass = nConf.pass else oldC.pass = _G["cfg"].pass
            end
            oldC.ip = nConf.ip
            ...
            nConf = nil
            _G["cfg"] = oldC
            local k, serNCfg = pcall(cjson.encode, oldC)
            if k then
                require("fopen").open("cfg",
                    function()
